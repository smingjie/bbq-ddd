...
public class FlowConfigDTO {
	...
	public FlowConfigDTO fetchNodes(List<FlowConfigAgg.NodeEntity> nodeEntities,
	                                List<FlowConfigAgg.HandlerEntity> handlers) {
	 ...
	 * @return
	 */
	public static FlowConfigAgg regroup(@NotNull FlowConfigDTO dto) {
		if (dto == null) {
			return null;
		}
		...
		dto.getNodes().forEach(o->{
			...
			if(o.getHandlers()!=null&&o.getHandlers().isEmpty()){
				handlerEntities.addAll(	Lists.transform(o.getHandlers() ,
						e->ModelUtils.convert(e, FlowConfigAgg.HandlerEntity.class)));
			}
		});
