package com.microserv.bbq.domain.dict;

import com.microserv.bbq.domain.common.factory.RepositoryFactory;
import com.microserv.bbq.infrastructure.persistence.repository.impl.DictRepositoryImpl;
import com.microserv.bbq.infrastructure.general.extension.annotation.ddd.DomainService;
import org.springframework.stereotype.Service;

import java.util.List;

/**
 * 领域模型-领域服务
 *
 * @author jockeys
 */
@Service
@DomainService
public class DictService {

	/**
	 * 根据字典值模糊查询匹配项
	 *
	 * @param likeValue 模糊匹配值
	 * @return 所有匹配到的字典实体记录
	 */
	public List<DictEntity> getByValue(String likeValue) {
		return RepositoryFactory.get(DictRepositoryImpl.class).selectByValue(likeValue);
	}
}
